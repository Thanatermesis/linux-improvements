			      ...
			      int datasync)
{
	...
	if (barrier_done < 0)
		return barrier_done;
	return err; /* generic_buffers_fsync_noflush returns 0 on success */
}

static ssize_t reiserfs_file_splice_read(struct file *in, loff_t *ppos,
...
static int reiserfs_file_mmap_prepare(struct vm_area_desc *desc)
{
			 ...
			 unsigned from, unsigned to)
{
	...
	unsigned long i_size_index = i_size_read(inode) >> PAGE_SHIFT;
	int new;
	int logit = reiserfs_file_data_log(inode);
	struct super_block *s = inode->i_sb;
	int bh_per_folio = folio_size(folio) / s->s_blocksize;
	struct reiserfs_transaction_handle th;
	int ret = 0;
	...
	if (logit) {
		...
		ret = journal_begin(&th, s, bh_per_folio + 1);
		if (ret)
	...
	for (bh = head = folio_buffers(folio), block_start = 0;
		...
		if (block_end <= from || block_start >= to) {
			...
			if (logit) {
				...
				/*
				 * do data=ordered on any folio past the end
				 * of file and any buffer marked BH_New.
				 */
				if (reiserfs_data_ordered(inode->i_sb) &&
				    (new || folio->index >= i_size_index)) {
...
#endif
	.mmap_prepare = reiserfs_file_mmap_prepare,
	.open = reiserfs_file_open,
	.release = reiserfs_file_release,
	.fsync = reiserfs_sync_file,
	.read_iter = generic_file_read_iter,
	.write_iter = generic_file_write_iter,
	.splice_read = reiserfs_file_splice_read,
